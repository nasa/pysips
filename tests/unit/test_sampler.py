...
def test_sample_default_kwargs(mocker):
...
def test_sample_custom_kwargs(mocker):
    mock_run_smc = mocker.patch(
        "bayesr.sampler.run_smc", return_value=("mock_models", "mock_likelihoods")
    )
...
def test_run_smc_functionality(mocker, multiproc):
    ...
    mock_prior = mocker.patch("bayesr.sampler.Prior", return_value=mock_prior_instance)
